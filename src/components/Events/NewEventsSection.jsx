...
export default function NewEventsSection() {
  ...
  const { data, isPending, isError, error } = useQuery({
    ...
    queryKey: ["events", { max: 2 }], // must be set so that React can cache data
    // if no cutom data needs to be passed than queryFn can simply point at the funtion that will be executed
    queryFn: ({ signal, queryKey }) => fetchEvents({ signal, ...queryKey[1] }), // ["events", { max: 2 }] to access second elem in array and spread
    //queryFn: ({ signal }) => fetchEvents({ signal, max: 2 }), // aqual http req code. It wants a fnc that return a Promise
